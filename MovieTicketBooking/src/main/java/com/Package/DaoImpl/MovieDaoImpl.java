	...
	/* @Override
	 public boolean insert(Movie movie) {
			...
			try {
				...
				System.out.println("inside MoviedaoImpl class inside insert method");
...
public boolean delete(long cinemaId) {
	...
	try {
		 ...
		 * 
		Session session = sessionFactory.getCurrentSession();
		Movie movie = session.byId(Movie.class).load(cinemaId);
		session.delete(movie);
		((Statement) session).executeUpdate("delete from cinemas where CNM_ID='" + cinemaId + "'");
		return true;
